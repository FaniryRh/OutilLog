...
@section('content')
    ...
    {!! Form::model($contact_company, ['method' => 'PUT', 'route' => ['admin.contact_companies.update', $contact_company->id], 'files' => true,]) !!}
        ...
        <div class="panel-body">
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($contact_company->logo)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$contact_company->logo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/'.$contact_company->logo) }}"></a>
                    @endif
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="contacts-region">
                    ...
                    @empty
                        @foreach($contact_company->contactsRegion as $item)
                            @include('admin.contact_companies.contacts_regions_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="groupe-sectoriel">
                    ...
                    @empty
                        @foreach($contact_company->listeGroupeSectoriel as $item)
                            @include('admin.contact_companies.liste_groupe_sectoriels_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
