...
@section('content')
    ...
    @can('asset_create')
    <p>
        <a href="{{ route('admin.assets.create') }}" class="btn btn-success fa fa-plus"> @lang('quickadmin.qa_add_new')</a>
                    ...
                    @if (count($assets) > 0)
                        @foreach ($assets as $asset)
                            <tr data-entry-id="{{ $asset->id }}">
                                ...
                                <td field-key='title' style="background-color: #ffeecc; font-weight: bold;">{{ $asset->title }}</td>
                                ...
                                <!-- <td field-key='notes'>{!! $asset->notes !!}</td> -->
                                                                <td>
                                    @can('asset_view')
                                    <a href="{{ route('admin.assets.show',[$asset->id]) }}" class="btn btn-xs btn-primary fa fa-eye"> @lang('quickadmin.qa_view')</a>
                                    @endcan 
                                    @can('asset_edit')
                                    <a href="{{ route('admin.assets.edit',[$asset->id]) }}" class="btn btn-xs btn-info fa fa-edit"> @lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('asset_delete')
{!! Form::open(array(
