...
@section('content')
    ...
    @can('capacite_create')
    <p>
        <a href="{{ route('admin.capacites.create') }}" class="btn btn-success">Nouvelle Capacité</a>
                        ...
                        @foreach ($capacites as $capacite)
                            <tr data-entry-id="{{ $capacite->id }}">
                                ...
                                <td field-key='piece_jointe'>@if($capacite->piece_jointe)<a href="{{ asset(env('UPLOAD_PATH').'/' . $capacite->piece_jointe) }}" target="_blank">Download file</a>@endif</td>
                                                                <td>
                                    @can('capacite_view')
                                    <a href="{{ route('admin.capacites.show',[$capacite->id]) }}" class="btn btn-xs btn-primary fa fa-eye">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('capacite_edit')
                                    <a href="{{ route('admin.capacites.edit',[$capacite->id]) }}" class="btn btn-xs btn-info fa fa-edit">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('capacite_delete')
{!! Form::open(array(
