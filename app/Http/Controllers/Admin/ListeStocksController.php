...
class ListeStocksController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $unites = \App\Unite::get()->pluck('nom', 'id')->prepend(trans('quickadmin.qa_please_select'), '');$entrees = \App\Entree::where('stock_id', $id)->get();$sorties = \App\Sortie::where('stock_id', $id)->get();$inventaires = \App\Inventaire::where('stock_id', $id)->get();

        $liste_stock = ListeStock::findOrFail($id);

        return view('admin.liste_stocks.show', compact('liste_stock', 'entrees', 'sorties', 'inventaires'));
    }
